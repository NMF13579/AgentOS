...
<!-- Переход = событие + guard + side effect. Без выполненного guard'а переход не выполняется. -->
...
| Session | VERIFYING             | VERIFICATION_PASSED  | нет блокеров                               | HANDOFF               | Обновить HANDOFF.md + project-status.md   |
...
1. Проверить Guard — если не выполнен, переход заблокирован, сообщить пользователю причину
2. Обновить поле state в нужном домене LAYER-3/STATE.md
3. Обновить поля last_event и last_updated
4. Дописать запись в Transition Log в STATE.md: YYYY-MM-DD: EVENT → STATE CHANGE
