...
Use `ROUTES-REGISTRY.md` to confirm ownership and routing.
...
1. **Single-Role Execution:** Every agent run must declare exactly one role in the task contract. Do not mix roles. Enforced by `scripts/check-single-role-execution.py`.
2. **Readiness Assertions:** No premature readiness claims allowed without explicit completion tokens. Enforced by `scripts/check-readiness-assertions.py`.
3. **YAML Safety:** Do not use empty JSON brackets `[]` in YAML blocks; leave the field empty or remove it.
4. **Idle-State Bypass:** Implement `is_idle_state()` bypass before parsing YAML frontmatter.
