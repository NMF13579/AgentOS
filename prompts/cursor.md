...
## Purpose
Reusable prompt pack for Cursor sessions working with AgentOS tasks.

## Use This Prompt When
Use this when you need Cursor-specific execution guidance for AgentOS tasks with strict guardrails.
...
- Cursor-specific usage note: use project files as source of truth.
- Use project files as source of truth.
- Do not rely on chat-only memory.
- Identify risk before mutation.
...
- Do not edit outside accepted task scope.
- Do not modify files outside task boundaries without explicit task scope.
...
- Do not mark completion if required validations fail.

## Evidence Requirements
- Preserve evidence of validation.
- Keep output markers in evidence records.
- Do not replace failed evidence with narrative claims.
...
- Do not modify Cursor config files.
