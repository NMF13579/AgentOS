...
- [ ] `SECTION:deploy` заполнен: окружение, домены, переменные, процедуры выката
- [ ] Пройден `CHECKLIST.md` перед релизом
- [ ] Секреты НЕ в репозитории — только имена переменных
- [ ] Владелец подтвердил: «Деплой готов, можно релизить»
