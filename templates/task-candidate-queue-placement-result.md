...
type: template
milestone: M54
template_for: task-candidate-queue-placement-result
status: draft
authority: template
...
This template describes a future queue placement result shape only; it does not create a queue artifact.

The template must default to QUEUE_PLACEMENT_BLOCKED and must not imply materialization success.

```json
{
  "queue_placement_result": {
    "result": "QUEUE_PLACEMENT_BLOCKED",
    "exit_code": 2,
    "materialized": false,
    "queue_entry_created": false,
    "queue_entry_path": null,
    "checked_candidate_id": "SOURCE_CANDIDATE_ID",
    ...
    "warnings": [],
    "blockers": [
      "QUEUE_PLACEMENT_BLOCKED_SAFE_DEFAULT"
    ],
    "boundary_flags": {
