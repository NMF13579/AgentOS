...
- Этап 3 (Medical Safety / split): `git mv` → `LAYER-1/UX-CHECKLIST-MEDICAL.md` (контент UX); отдельный redirect `ux-checklist-medical.md` на FS без учёта регистра с тем же именем несовместим с каноном — см. `CHANGELOG.md` [docs-medical-ux-canonical-file]; в `MEDICAL-ROLES-AND-PERMISSIONS.md` добавлена секция «Ролевой медицинский UX (экраны)»; перекрёстные ссылки обновлены (`START.md`, `agents.md`, `LEGAL-152FZ.md`, `DOMAIN-ADAPTER.md`, `ux-checklist-core.md`, LAYER-2, `template-sync-index.md`, LAYER-3 `project-status.md`). `MEDICAL-SAFETY.md`, `LEGAL-152FZ.md`, `security.md` — формулировки autonomous clinical use / real patient data in LLM согласованы.
- Этап 2 (онбординг человека): в `README.md` Quick Start сведён к одной CTA на `START.md`; medical disclaimer — в blockquote по шаблону; install policy (main vs dev) уже была в разделе Installation. `QUICK-START-NOVICE.md`, `QUICK-START.md`, `ONBOARDING-WIZARD.md` — вводные блоки уже соответствовали ТЗ, без правок.
- Этап 1 (коррекция docs, текущий прогон): синхронизация маршрутов агента — в `llms.txt` каноническое предупреждение в blockquote; в `START.md` уточнены backticks для блока AI-agent; `AGENTS.md` / `CLAUDE.md` / `GEMINI.md` — единый блок metadata (`NOT A POLICY SOURCE`, canonical bootstrap → `llms.txt`), тело без императива «Read…», только отсылки; `FAQ.md` проверен (цепочка контекста и ссылки на `owner` / `stack-presets` / `task-protocol` / `LAYER-2/qa/` — ок).
