# memory-bank/project-status.md

Non-runtime project note.

Use `state/MAIN.md` for current state and recovery.
Use `workflow/MAIN.md` for execution boundaries.
Use `quality/MAIN.md` for proof before completion.

Current summary:

- Milestone 40 (External Portability & Installer MVP) is COMPLETE (2026-05-14).
- Milestone 36 (External MVP Usability) is COMPLETE (2026-05-10).
- All P0 external usability and portability blockers REPAIRED.
- Project state: `M40_EXTERNAL_PORTABILITY_SAFE`.
- Pipeline status: GREEN (`PASS` for all core validators).
- Major Implementation:
    - Interactive `install.sh` script for safe local installation without overwriting existing files.
    - Fixed `--help` semantics to avoid side effects in clean templates.
    - Updated documentation for safe dotfile copying (`cp -a`).
    - Clarified command execution paths (Mode A and Mode B) for first-user guidance.
- AgentOS now routes runtime behavior through five canonical modules.
- `llms.txt` is the only agent startup entry.
- `ROUTES-REGISTRY.md` records canonical module ownership.
